// ==========================================
// 🛡️ [W3C 표준 최첨단 보안 캡처링 쉴드] 
// 브라우저 하드웨어 신호가 도달하기 전 공중에서 우클릭과 F12를 완전 파괴합니다.
// ==========================================
(function() {
    // 1. 우클릭 원천 차단 (true 지정을 통해 캡처링 단계에서 가로챔)
    window.addEventListener('contextmenu', function(e) {
        e.preventDefault();
        e.stopPropagation();
    }, true);

    // 2. 개발자 도구 단축키 전면 마비 (e.key 최신 표준 규격 적용)
    window.addEventListener('keydown', function(e) {
        // F12 차단
        if (e.key === 'F12') {
            e.preventDefault(); e.stopPropagation(); return false;
        }
        // Ctrl + Shift + I (개발자도구) / J (콘솔창) / C (요소검사) 차단
    ...
    }, true); // 👈 true 옵션이 핵심! 신호가 하위 엘리먼트로 내려가기 전에 차단!
})();

// ==========================================
// 🛡️ 지도가 화면 찢어지는 현상 차단 CSS
// ==========================================
...
masterShield.innerHTML = `
    ...
    /* 암호창 입력 칸에서도 우클릭/복사를 전면 금지합니다 */
    #bio-modal-input { user-select: none !important; -webkit-user-select: none !important; }
...
// ==========================================
// 🚪 [최상위 보안 게이트웨이 기동]
// DOMContentLoaded 이벤트 캡처링 단계에서 브라우저보다 먼저 가로채 무력화합니다.
// ==========================================
